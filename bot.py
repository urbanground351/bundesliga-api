...
def fetch_matches():
    # Kambi CDN - Tüm Almanya Futbol Ağı
    url = "https://eu-offering-api.kambicdn.com/offering/v2018/ub/listView/football/germany.json?lang=tr_TR&market=TR"
    
    final_data = {
        "last_updated": datetime.now().isoformat(),
        "source": "Kambi CDN (Bundesliga)",
        "matches": []
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for event_data in data.get("events", []):
                ...
                # FİLTRE: İçinde "bundesliga" geçenleri al ama "2." veya "3." ligleri çöpe at
                path_names = [str(p.get("name", "")).lower() for p in event.get("path", [])]
                is_bundesliga = any("bundesliga" in p and "2." not in p and "3." not in p for p in path_names)
    ...
    with open("matches.json", "w", encoding="utf-8") as f:
        ...
        print(f"İşlem Tamam! {len(final_data['matches'])} Bundesliga maçı başarıyla çekildi.")
